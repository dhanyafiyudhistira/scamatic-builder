...
test('Builder replaces the Isaac rollout setup with audited runtime worker modes', async () => {
  ...
  assert.match(builder, /apiRequest\('\/health\/data-plane\/ready'\)/)
  assert.match(builder, /window\.setTimeout\(refreshHealth, 5000\)/)
  assert.match(builder, /window\.clearTimeout\(retryTimer\)/)
  ...
  assert.match(projectsHandler, /MONGO_TRANSACTIONS_REQUIRED[^\n]*res\.status\(503\)/)
